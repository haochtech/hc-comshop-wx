...
Component({
    ...
    methods: {
        addImg: function() {
            var l = this,
                s = this.data.imgGroup;
            wx.chooseImage({
                count: this.data.imgMax - s.length,
                success: function(g) {
                    l.triggerEvent("on-chooseImage"), (s = s.concat(g.tempFilePaths)).length > l.data.imgMax && s.splice(l.data.imgMax), l.setData({
                        imgGroup: s
                    });
                    var p = g.tempFilePaths.length;
                    a = s.length;
                    for (var i = 0; i < g.tempFilePaths.length; i++)! function(i) {
                        var s = g.tempFilePaths[i].split(".")[g.tempFilePaths[i].split(".").length - 1],
                            n = (new Date).getTime(),
                        ...
                        r[i + a - p] = wx.uploadFile({
                            ...
                            success: function(r) {
                                t[i + a - p] = JSON.parse(r.data).image_o, l.setData(e({}, "progressList[" + (i + a - p) + "]", 100)), l.triggerEvent("on-changeImage", {
                                    value: t,
                                    len: a
                                })
                            }
        ...
        remove: function(e) {
            ...
            n[i] < 100 && r[i].abort(), s.splice(i, 1), r.splice(i, 1), n.splice(i, 1), t.splice(i, 1), a = s.length, this.setData({
                imgGroup: s,
                progressList: n
